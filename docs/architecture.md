...
图中箭头表示编译期依赖。适配器若要实现引擎定义的端口，可以依赖 `procora-engine` 的轻量 `ports` 模块；引擎本身不得反向依赖适配器。若端口数量增长，应将其提取为独立的 `procora-ports` crate，避免形成循环。

## 4. 建议 Cargo workspace

仓库已经迁移为以下 Cargo workspace；各 crate 当前处于骨架或首个纵向切片阶段：
...
├── Cargo.lock
├── crates/
│   ├── procora-core/          # 领域类型、任务图、状态和值对象
│   ├── procora-engine/        # 命令、事件循环、调度、对账与端口
│   ├── procora-config/        # 多格式读取、合并、校验与规范化
│   ├── procora-source/        # 本地监听及后续 Git/HTTP 定义源
│   ├── procora-process/       # 子进程启动、停止、退出与进程树
│   ├── procora-monitor/       # 资源采样与聚合
│   ├── procora-log/           # 输出采集、索引、滚动与订阅
│   ├── procora-storage/       # 状态快照、运行元数据与迁移
│   ├── procora-platform/      # Linux/macOS/Windows 系统适配
│   ├── procora-protocol/      # 版本化 IPC DTO 与编解码
│   ├── procora-tui/           # 终端状态、页面、组件与输入映射
│   ├── procora-cli/           # 命令定义、输出格式与客户端
│   └── procora-daemon/        # 组合根、后台服务与 IPC 服务端
├── tests/                     # 跨 crate 共享测试夹具
└── docs/
```

根 manifest 是虚拟 workspace，不再声明 package。`procora` 二进制位于 `crates/procora-cli/src/main.rs`；各 crate 的公共行为测试放在自身 `tests/`，根 `tests/` 只保存确需跨 crate 复用的夹具。

应在实际需求出现时才增加 crate。单个 crate 内先按文件模块拆分，只有当编译边界、复用关系或平台隔离足够明确时才继续拆包，防止早期工作区碎片化。
...
| `procora-config` | 格式探测、Raw DTO 反序列化、字段诊断、路径规范化、校验、图编译 | 运行任务、监听 UI；变量与合并仍属后续能力 |
| `procora-source` | 发现、监听、拉取任务定义，生成带版本的候选输入 | 直接应用配置或修改引擎状态 |
...
同一主版本内新增字段应保持向后兼容；未知事件可忽略但必须保留序列连续性。客户端断线重连时先读取快照，再从事件序列号续订；若游标已过期则重新获取快照。
