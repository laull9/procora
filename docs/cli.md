...
2. 总览使用上下方向键或 `j/k` 选择，`s/x/r` 启动、停止或重启，连续两次 `d` 移除注册；`Enter` 进入原有 Task 详情 TUI，详情中的 `q/Esc` 返回总览。
3. 显式 PATH 时优先按规范化目录连接已注册服务；目录尚未注册时，在全局服务器中发现配置并注册服务。
4. 显式 PATH 且全局服务器不存在时，仍询问启动全局服务或创建仅与本次 TUI 同生命周期的临时 `ServiceHost`。
