...
- “包文件”集中完成构建、打开、验证、解包、安装、临时运行、裸机部署和命名导出推送；从某个 Service 进入时自动绑定其目录作为构建上下文。连续两次按 `Delete` 或大写 `X` 可永久删除当前 `.pcpkg`，即使其清单已经损坏也保留该恢复出口。
- “已安装”集中查看 active/pending release 与原始包；`R` 回滚最近历史版本，`c` 收敛中断安装，连续两次大写 `U` 解除包托管但保留数据，连续两次大写 `D` 永久删除该安装目录。

左右方向键移动当前折叠文本，`F3` 切换全部长路径、摘要和错误信息的自动横移；触控板横向滚动沿用相同语义。操作离开全屏页面后显示既有的阶段进度与交互向导，完成或失败都会返回原位置并给出结果。构建和解包遇到同名目标时自动选择带编号的新路径，不会隐式覆盖。
