...
骨架已经提供 `notify_ready` 与要求显式选择用户/系统总线的 `list_unit_names`，但默认不启用 feature。`procora enable/disable` 不依赖这个可选 D-Bus 层，而是生成固定的 systemd user unit 并调用 `systemctl --user` 完成加载、启停和移除。Linux CI 单独编译并测试该 feature，且用隔离假 `systemctl` 覆盖权限失败回滚；更广泛的外部 unit 控制、真实 polkit 策略和 D-Bus 信号订阅仍需独立集成环境。
