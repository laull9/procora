...
在服务总览按 `p` 可进入包工作台，统一完成包文件操作与已安装 release 的审计、恢复和清理。包文件页按 `B` 可一体完成构建、部署预检、确认和裸机部署，`b` 与 `d` 继续提供独立构建和当前包部署；新包会自动选中，取消预检不会修改远端。包文件可二次确认直接删除；同名普通 Service 不会阻塞损坏包安装的独立清理，也不会被误解除。完整格式、确定性边界和使用流程见 [Procora Service 包](docs/packages.md)。
