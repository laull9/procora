...
- [x] 实现进程外 exec/HTTP GET readiness、连续阈值、超时/取消处理和真实 `healthy` 依赖门控。
- [x] 支持自定义成功退出码，并统一接入重启和 `completed_successfully` 判断。
- [x] 提供 Bash、Zsh、Fish、PowerShell 和 Elvish 的 shell completion 输出命令。
- [x] 提供包含默认值和规范化路径、无运行副作用的有效配置 JSON 输出。
- [x] 支持有界显式 Task `env_file`，纳入闭包修订、热更新监听和进程身份差异，不默认读取 `.env`。
- [x] 支持 `max_restarts` 与稳定运行重置窗口，达到上限后停止创建风暴并允许原地放宽策略恢复。
- [x] 将资源采样与状态刷新解耦，并以单次系统刷新批量聚合一个 Service 的全部活动 Task 进程树。
...
- [x] 对单入口 notify 事件去抖，重读完整文件并生成稳定内容修订和语义差异。
- [x] 扩展安全受限的配置 include，并在任一成员变化后重读、校验和修订整个闭包。
- [x] 实现配置候选修订、CLI 预览/确认、TOCTOU 校验、策略原地更新和失败保留旧修订。
- [x] 按语义差异对新增、删除和身份变化 Task 做细粒度对账，并在失败时恢复旧图且保留无影响运行身份。
- [x] 通过受控 Python 子进程支持显式 `procora.py`，限制执行、输出和环境并纳入候选修订。
- [x] 实现固定提交、资源有界的 Git 任务定义源，远程内容只产生可重新确认的候选。
- [x] 在 Linux CI 单独验证 `--features systemd`，补显式用户/系统总线和权限失败回滚测试。
...
- [x] 已实现项目级 `env`、`task_defaults`、命令文本、精确 `command` argv 简写和字段/环境来源追踪，并由 effective config 完整展开。
- [x] TUI 直接编辑项目环境、Task 默认层、`env_file`、exec/HTTP 健康检查、成功退出码和依赖条件；继承值与声明层环境不会被展开写回，JSON 值保证精确往返。
- [x] 为配置演进建立旧写法兼容夹具，确保 YAML、TOML、JSON 与 Python 生成配置得到一致领域语义。
- [x] 实现可组合的命名 Task 模板，固定 map/标量/list/argv 覆盖顺序、继承链诊断、具体来源和 TUI 选择/预览，不引入隐式参数追加。
- [x] 实现显式 profile：持久保存活动场景与 Task 白名单，固定共享层覆盖优先级和来源，TUI 可切换并立即重编译预览且不丢未准入声明。
- [x] 支持 profile 单继承链和精确循环诊断，TUI 可结构化新增、编辑、重命名及删除 profile，并同步活动选择和继承引用。
- [x] 实现显式 `${vars.NAME}` 项目变量：支持链式引用、转义和循环诊断，覆盖常用执行/环境/健康字段，effective config 可解释且 TUI 保存不展开表达式。
- [x] 简化 `depends_on`：支持默认条件名称数组、条件标量和旧版对象，兼容 process-compose 条件别名，并由 TUI 确定性写回紧凑形式。
- [x] 为生命周期与健康检查增加 `h`/`m`/`s`/`ms` 可组合时长，兼容旧 `_ms` 整数，并由 TUI 统一读写可读字段。

## 工程质量

- [x] 加入 cargo-deny、RustSec/cargo-audit 和依赖许可证、来源策略。
- [x] 加入配置/状态机性质测试与高速日志压力测试。
- [x] 加入每周真实 Task 生命周期 soak 与 Unix 文件描述符增长门禁。
- [x] 固定最低 Rust 版本 CI，升级 sysinfo 等依赖时检查 MSRV。
- [x] 为公共配置模式、IPC 协议和持久化格式建立 ADR 与兼容性策略。
