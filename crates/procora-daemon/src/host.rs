...
use procora_config::CompiledProject;
use procora_core::{ProjectSpec, TaskId};
use procora_engine::{Engine, EngineCommand, EngineEffect, RuntimeEvent, TaskRunIdentity};
use procora_log::{FileLogStore, LogStream, TailBuffer};
use procora_monitor::SystemMonitor;
use procora_process::{ManagedChild, spawn_task};
use procora_protocol::{LogBatchDto, LogCursorDto, ProjectSnapshot, SnapshotSourceDto, TaskView};
use thiserror::Error;
...
impl ServiceHost {
    ...
    pub fn append_service_log(&self, bytes: &[u8]) -> Result<(), procora_log::FileLogError> {
        self.file_logs
            .as_ref()
    ...
    pub fn read_task_log(
        ...
        max_bytes: usize,
    ) -> Result<LogBatchDto, procora_log::FileLogError> {
        let files = self.file_logs.as_ref().ok_or_else(|| {
            procora_log::FileLogError::Io(std::io::Error::new(
        ...
        let batch = files.read_task(
            task_id,
            cursor.map(|cursor| procora_log::FileLogCursor {
