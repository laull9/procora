...
use procora_config::{DiscoveryError, discover_path};
...
impl Center {
    ...
    pub fn load(repository: SqliteCenterRepository) -> Result<Self, CenterError> {
        ...
        for stored in stored_services {
            let service = crate::managed::restore_service(stored);
    ...
    fn task_logs(
        &mut self,
        selector: &ServiceSelectorDto,
        task_id: &procora_core::TaskId,
        cursor: Option<procora_protocol::LogCursorDto>,
        max_bytes: u32,
    ) -> Result<procora_protocol::LogBatchDto, CenterError> {
        let name = self.resolve_name(selector)?;
        let service = self.services.get_mut(&name).expect("名称已经解析");
        let host = service
            .host
