use procora_protocol::{SnapshotSourceDto, TaskStatusDto, TaskView};
use ratatui::{
    Frame,
...
fn render_header(frame: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    let rows = Layout::default()
        ...
        .split(inner);
    let (source, source_color) = source_label(app.snapshot().source);
...
fn render_tasks(frame: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    render_task_details(frame, panes[1], app.selected_task());
}
...
fn render_task_list(frame: &mut Frame<'_>, area: Rect, app: &App) {
    let items = app
        ...
        .iter()
        .map(|task| {
            let (symbol, color) = status_visual(task.status);
    ...
    let list = List::new(items)
        .block(
            Block::default()
                .title(format!("任务 · {}", app.snapshot().tasks.len()))
                .borders(Borders::ALL),
        )
        .highlight_symbol("› ")
        .highlight_style(Style::default().fg(ACCENT).add_modifier(Modifier::BOLD));
...
fn render_task_details(frame: &mut Frame<'_>, area: Rect, task: Option<&TaskView>) {
    let content = task.map_or_else(
        || Text::from("配置中没有任务。\n添加任务后重新打开 TUI。"),
        |task| {
            let (_, status_color) = status_visual(task.status);
            ...
            let (cpu, memory) = resource_labels(task);
            Text::from(vec![
                detail_line("任务", task.task_id.as_str()),
                detail_line("命令", &task.command),
                Line::from(vec![
                    Span::styled("状态  ", Style::default().fg(Color::DarkGray)),
                    Span::styled(status_label(task.status), Style::default().fg(status_color)),
                ]),
                detail_line("依赖", &dependencies),
                detail_line("CPU", &cpu),
                detail_line("内存", &memory),
                Line::default(),
                detail_line(
                    "说明",
                    task.message.as_deref().unwrap_or("尚无额外运行信息"),
                ),
            ])
        },
    );
    ...
    for task in &app.snapshot().tasks {
        if task.dependencies.is_empty() {
            ...
            for dependency in &task.dependencies {
                lines.push(graph_line(
                    format!("{} ──▶ {}", dependency, task.task_id),
                    selected == Some(&task.task_id),
                ));
            }
        }
...
fn render_logs(frame: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    let message = match app.snapshot().source {
        SnapshotSourceDto::ConfigPreview => {
            "当前是本地配置预览，尚未连接服务宿主。\n\n进程运行后，这里将按游标显示 stdout、stderr 和系统事件。"
        }
        SnapshotSourceDto::EmbeddedLive => {
            "当前连接随 TUI 生命周期运行的嵌入式服务宿主。\n\n退出 TUI 时，该宿主也会停止。"
        }
        SnapshotSourceDto::CenterLive => "已连接中心服务器，正在等待该任务的日志帧。",
        SnapshotSourceDto::CenterStale => {
            "中心服务器连接已中断，现有日志可能过期。\n\n重新连接后将从最后游标继续读取。"
        }
    };
...
fn render_footer(frame: &mut Frame<'_>, area: Rect, app: &App) {
    let live = app.snapshot().source == SnapshotSourceDto::CenterLive;
    let controls = if live && app.control_allowed() {
        "↑↓/jk 选择  Tab/←→ 切页  s 启动  x 停止  r 重启  q/Esc 退出"
    } else {
        "↑↓/jk 选择任务  Tab/←→ 切换页面  1/2/3 直达  q/Esc 退出"
    };
    let feedback = app.feedback().unwrap_or(if live && app.control_allowed() {
        "中心前端模式：可直接管理服务，页面会自动增量刷新"
    } else if live {
        "中心前端模式：当前会话仅允许查看，页面会自动增量刷新"
    } else {
        "当前宿主随 TUI 生命周期运行"
    });
    let footer = Paragraph::new(vec![Line::from(controls), Line::from(feedback)])
...
fn render_too_small(frame: &mut Frame<'_>, area: Rect) {
    let message = Paragraph::new("终端尺寸过小\n请扩大到至少 30×8")
        .alignment(Alignment::Center)
        .block(Block::default().title("Procora").borders(Borders::ALL));
...
/// 创建依赖图中的一行并按选择状态着色。
fn graph_line(content: String, selected: bool) -> Line<'static> {
...
/// 返回快照来源标签及颜色。
const fn source_label(source: SnapshotSourceDto) -> (&'static str, Color) {
    match source {
        SnapshotSourceDto::ConfigPreview => ("本地预览 · 未连接", Color::Yellow),
        SnapshotSourceDto::EmbeddedLive => ("嵌入式宿主 · 随 TUI 退出", Color::Green),
        SnapshotSourceDto::CenterLive => ("中心服务器实时", Color::Green),
        SnapshotSourceDto::CenterStale => ("连接中断 · 数据过期", Color::Red),
    }
}

/// 返回任务状态的符号与颜色。
const fn status_visual(status: TaskStatusDto) -> (&'static str, Color) {
    match status {
...
/// 返回任务资源的可读标签。
fn resource_labels(task: &TaskView) -> (String, String) {
