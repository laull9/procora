...
use procora_config::{DiscoveredProject, discover_path};
use procora_daemon::ServiceHost;
use procora_source::DependencyManager;
...
pub(crate) fn edit(path: Option<&Path>) -> anyhow::Result<()> {
    ...
    procora_tui::edit_config(&discovered.config_path).context("配置编辑器退出失败")
}
...
pub(crate) fn edit_after_init(path: &Path, no_edit: bool) -> anyhow::Result<()> {
    ...
    procora_tui::edit_config(path).context("配置编辑器退出失败")
}
